...
export class TradeBehavior extends BehaviorState {
  ...
  constructor(target: Agent, nextState?: () => BehaviorState) {
    ...
    this._tradeFavor = Math.random() > 0.5 ? 0 : -1;
  }
  ...
  decideNextTradeAction(state: number) {
    ...
    if (state === 1) {
      ...
      for (const req of requests) {
        const index = this._responded.indexOf(req);
        if (index === -1 && !req.pass && !KB.is.neededForQuest(req.model) && status.favor > this._tradeFavor) {
          // potentially offer
          if (
            ...
            let offer = undefined;
            if (req.model instanceof Item) {
              offer = KB.get.inventory.find((item) =>
                item.sameAs(req.model as Item)
              );
            } else if (req.model instanceof Info) {
              const answer = KB.get.answerToQuestion(req.model);
              offer = answer
                ? {
                    answer,
                    question: req.model,
    ...
    // ready trade
    if (state === 2 && !ready && status.total > 0 && status.favor >= this._tradeFavor) {
   ...
   * if < 0: favors other
   */
  tradeStatus(): { total: number; favor: number } {
    const myOffers = KB.get.numberOfOffers();
    const otherOffers = KB.get.numberOfOffersByOtherAgent();
    return { total: myOffers + otherOffers, favor: myOffers - otherOffers };
  }

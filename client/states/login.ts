...
export class Login extends Phaser.State {
  ...
  public create(): void {
    ...
    // see if url injected character name exists
    //  URL?{"user":"name"}
    let urlParam: any = {};
    try {
      urlParam = JSON.parse(unescape(document.URL.split("?")[1]));
    } catch (err) {}
    if (urlParam && urlParam.user && typeof urlParam.user === "string") {
      userField.setText(urlParam.user);
    }
    ...
    const login = () => {
      ClientAPI.login(userField.value, "" /*passwordField.value*/)
        .then((res) => {
